...
// 获取指定key的Hash表的指定的field的值，并将其赋值给value对象
// key:key
// fields: 字段名列表
// value:对象
// 返回值:
// exists:是否存在此此项
// err:错误对象
func (this *RedisPool) HMGet(key string, value interface{}, fields ...string) (exists bool, err error) {
	conn := this.GetConnection()
	defer conn.Close()

	fieldNum := len(fields)
	reply, err1 := redis.Values(conn.Do("HMGET", redis.Args{}.Add(key).AddFlat(fields)...))
	if err1 != nil {
		err = err1
		return
	}
	...
	// 构造参数
	newReply := make([]interface{}, 0, 2*len(reply))
	var i int
	for i = 0; i < fieldNum; i++ {
		newReply = append(newReply, []byte(fields[i]))
		newReply = append(newReply, reply[i])
	}

	if err = redis.ScanStruct(newReply, value); err != nil {
		return
	}
